    ...
    public class BakeTool : SplineTool
    {
        ...
        private void Bake(MeshGenerator gen)
        {
            ...
            if(format == BakeMeshWindow.SaveFormat.OBJ)
            {
                ...
                FileInfo[] files = dirInfo.GetFiles(filter.gameObject.name + "*.obj");
            ...
            {
                dirInfo = new DirectoryInfo(savePath);
                FileInfo[] files = dirInfo.GetFiles(filter.gameObject.name +"_"+ filter.gameObject.transform.parent.name + "*.asset");
                string meshName = filter.gameObject.name + "_" + filter.gameObject.transform.parent.name;
